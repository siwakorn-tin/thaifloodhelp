...
⚠️ กฎสำคัญที่สุด - ห้ามสร้างข้อมูลเอง:
- แยกเฉพาะข้อมูลที่มีอยู่จริงในข้อความเท่านั้น
- ห้ามคาดเดา ห้ามสมมติ ห้ามเติมข้อมูลที่ไม่มีในข้อความ
- ถ้าข้อมูลใดไม่ปรากฏในข้อความ ให้ใช้ค่าว่าง (empty string, empty array, หรือ 0)
- ห้ามใส่ตัวอย่าง ห้ามใส่ข้อความอธิบาย
- นี่คือระบบช่วยเหลือผู้ประสบภัย ความถูกต้องเป็นเรื่องสำคัญที่สุด

กฎสำหรับการจัดระดับความเร่งด่วน (urgency_level):
...
วิธีการแยกข้อมูล:
- ชื่อและนามสกุล: แยกเฉพาะที่ระบุชัดเจนในข้อความ (ถ้าไม่มี ให้เป็นค่าว่าง)
- ชื่อผู้รายงาน: มักมาจากชื่อโปรไฟล์ผู้โพสต์หรือลายเซ็นในข้อความ (ถ้าไม่มี ให้เป็นค่าว่าง)
- เวลาติดต่อล่าสุด: เฉพาะที่ระบุชัดเจน เช่น "วันที่ 22" "เมื่อวาน" (ถ้าไม่มี ให้เป็นค่าว่าง)
- เบอร์โทรศัพท์: เก็บเป็น array เฉพาะเบอร์ที่มีอยู่จริง (ถ้าไม่มี ให้เป็น array ว่าง)
- ที่อยู่: คัดลอกที่อยู่ที่ระบุในข้อความตามที่เป็นอยู่ (ถ้าไม่มี ให้เป็นค่าว่าง)
- จำนวนคน: นับเฉพาะที่ระบุชัดเจน (ถ้าไม่มี ให้เป็น 0)
- ภาวะสุขภาพ: เฉพาะที่ระบุชัดเจน เช่น "ป่วย" "ติดเตียง" (ถ้าไม่มี ให้เป็นค่าว่าง)
- ความช่วยเหลือที่ต้องการ: เฉพาะที่ขอมา เช่น "เรือ" "อาหาร" (ถ้าไม่มี ให้เป็นค่าว่าง)
- ข้อมูลเพิ่มเติม: บันทึกข้อมูลสำคัญอื่นๆ ที่ไม่อยู่ในช่องอื่น (ถ้าไม่มี ให้เป็นค่าว่าง)

จำไว้: ถ้าไม่แน่ใจ ใช้ค่าว่างดีกว่าข้อมูลที่ไม่ถูกต้อง`;
